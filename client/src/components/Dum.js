...
class Dum extends Component {
    constructor(props) {
        super(props);
        this.state = { queryRes:'',queryModified:'',arrivee: (new Date().toISOString().split('T')[0]), navire: '',  portdechrgmt: '', importateur: '', produit: '',quantite:'', navireList: [], importateurList: [], produitList: [], portdechrgmtList: [] }
    ...
    getElementsName() {
        ...
        axios.post("http://localhost:5000/dum", { arrivee })
            .then(response => {
    ...
    handleFormSubmit(event) {
        ...
        axios.post("http://localhost:5000/dum/register", { arrivee,navire,portdechrgmt,importateur,produit,quantite })
            .then(response => {
                console.log('server response if integration done',response.data)
                this.setState({ queryRes:'',queryModified:'',arrivee: (new Date().toISOString().split('T')[0]), navire: '',  portdechrgmt: '', importateur: '', produit: '',quantite:'', navireList: [], importateurList: [], produitList: [], portdechrgmtList: [] })
                this.getElementsName()
            })
            .catch(err=>console.log('error from'))
        }
    render() {
        return (
            <div>
